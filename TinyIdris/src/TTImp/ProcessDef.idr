...
getRHSEnv env (Bind n (PVar _ ty) sc) (Bind n' (PVTy _) scty) with (nameEq n n')
  getRHSEnv env (Bind n (PVar _ ty) sc) (Bind n' (PVTy _) scty) | Nothing
      = throw (GenericMsg "Can't happen: names don't match in getRHSEnv")
  getRHSEnv env (Bind n (PVar _ ty) sc) (Bind n (PVTy _) scty) | (Just Refl)
      = getRHSEnv (PVar n ty :: env) sc scty
